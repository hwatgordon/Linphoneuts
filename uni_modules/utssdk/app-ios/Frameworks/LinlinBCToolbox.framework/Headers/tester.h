...
typedef struct {
	...
	int average_time; /* average time that takes the suite. Used for sorting. Use 0 if unknown, in which case test no
	                     sorting is done. */
	int cpu_weight;   /* rough cpu resource required compared to other suites. The default-initiliazed 0 value is
	                    interpreted as 1   If a suite is expected to require twice more cpu resources as the average of
	                    suites, it should indicate a   cpu_weight of 2, so that the bc_tester framework takes it into
	                    account by reducing the workload due to other   suites (ie reducing the number of parallel process
	                    runing suites)*/
} test_suite_t;
...
BCTBX_PUBLIC void bc_tester_add_suite(
    test_suite_t *suite); /* Add suite to the list of suites to be executed. If average_time is provided, the list are
                             automatically sorted so that longer suites are executed first, in order to optimize
                             (minimize) the total execution time when suites are run in parallel mode.*/
...
#define BC_ASSERT_PTR_EQUAL(actual, expected)                                                                          \
	bc_assert(__FILE__, __LINE__, ((actual) == (expected)), "BC_ASSERT_PTR_EQUAL(" #actual "!=" #expected ")")
#define BC_ASSERT_PTR_NOT_EQUAL(actual, expected)                                                                      \
	bc_assert(__FILE__, __LINE__, ((actual) != (expected)), "BC_ASSERT_PTR_NOT_EQUAL(" #actual "==" #expected ")")
#define BC_ASSERT_PTR_NULL(value) bc_assert(__FILE__, __LINE__, ((value) == NULL), "BC_ASSERT_PTR_NULL(" #value ")")
#define BC_ASSERT_PTR_NOT_NULL(value)                                                                                  \
	bc_assert(__FILE__, __LINE__, ((value) != NULL), "BC_ASSERT_PTR_NOT_NULL(" #value ")")

#define BC_ASSERT_STRING_NOT_EQUAL(actual, expected)                                                                   \
	_BC_ASSERT_PRED("BC_ASSERT_STRING_NOT_EQUAL", (strcmp((const char *)(cactual), (const char *)(cexpected))),        \
	                actual, expected, const char *, "Expected NOT %s but it was.", cexpected)
...
#define BC_ASSERT_DOUBLE_EQUAL(actual, expected, granularity)                                                          \
	_BC_ASSERT_PRED("BC_ASSERT_DOUBLE_EQUAL",                                                                          \
	                ...
	                double, "Expected %f but was %f.", cexpected, cactual)
#define BC_ASSERT_DOUBLE_NOT_EQUAL(actual, expected, granularity)                                                      \
	_BC_ASSERT_PRED("BC_ASSERT_DOUBLE_NOT_EQUAL",                                                                      \
	                ((fabs((double)(cactual) - (cexpected)) > fabs((double)(granularity)))), actual, expected, double, \
	                "Expected %f but was %f.", cexpected, cactual)

#define BC_ASSERT_EQUAL(actual, expected, type, type_format)                                                           \
	_BC_ASSERT_PRED("BC_ASSERT_EQUAL", ((cactual) == (cexpected)), actual, expected, type,                             \
	                "Expected " type_format " but was " type_format ".", cexpected, cactual)
#define BC_ASSERT_NOT_EQUAL(actual, expected, type, type_format)                                                       \
	_BC_ASSERT_PRED("BC_ASSERT_NOT_EQUAL", ((cactual) != (cexpected)), actual, expected, type,                         \
...
#define BC_ASSERT_GREATER(actual, min, type, type_format)                                                              \
	_BC_ASSERT_PRED("BC_ASSERT_GREATER", ((cactual) >= (cexpected)), actual, min, type,                                \
	                "Expected at least " type_format " but was " type_format ".", cexpected, cactual)
#define BC_ASSERT_LOWER(actual, max, type, type_format)                                                                \
	_BC_ASSERT_PRED("BC_ASSERT_LOWER", ((cactual) <= (cexpected)), actual, max, type,                                  \
	                "Expected at most " type_format " but was " type_format ".", cexpected, cactual)
#define BC_ASSERT_GREATER_STRICT(actual, min, type, type_format)                                                       \
	_BC_ASSERT_PRED("BC_ASSERT_GREATER", ((cactual) > (cexpected)), actual, min, type,                                 \
	                "Expected more than " type_format " but was " type_format ".", cexpected, cactual)
